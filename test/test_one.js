...
describe("Variables", function () {
  ...
  describe("test if unlockTime variable  declared", function () {
    it("is declared?", async function () {
      const { variablesContract, unlockTime } = await loadFixture(deployVvariablesContract);
      expect(await variablesContract.unlockTime()).to.equal(unlockTime);
    });
  });

  describe("test if owner variable declared", function () {
    it("is declared?", async function () {
      const { variablesContract, owner } = await loadFixture(deployVvariablesContract);
      expect(await variablesContract.owner()).to.equal(owner.address);
    });
  });
